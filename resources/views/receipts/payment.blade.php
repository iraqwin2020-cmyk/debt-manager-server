...
<body>
    ...
    <table>
        <tr><th>المبلغ المسدَّد</th><td><span style="display:inline-flex;align-items:center;gap:4px" dir="ltr"><span>{{ $payment->debt->currency === 'USD' ? '$' : 'د.ع' }}</span><bdi>{{ number_format($payment->amount) }}</bdi></span></td></tr>
        ...
        <tr><th>المتبقي على الدين</th><td><span style="display:inline-flex;align-items:center;gap:4px" dir="ltr"><span>{{ $payment->debt->currency === 'USD' ? '$' : 'د.ع' }}</span><bdi>{{ number_format($payment->debt->amount - $payment->debt->paid_amount) }}</bdi></span></td></tr>
