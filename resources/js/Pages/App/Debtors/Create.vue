...
<template>
    <AppLayout>
        <div class="mx-auto max-w-lg">
            <h1 class="mb-6 text-lg font-extrabold sm:text-2xl">إضافة عميل جديد</h1>
            ...
            <form class="space-y-4 rounded-card p-6" style="background: var(--surface-card); box-shadow: var(--shadow-card)" @submit.prevent="submit">
                ...
                <div>
                    <label class="mb-1 block text-sm font-semibold">العنوان (اختياري)</label>
                    <input v-model="form.address" type="text" class="w-full rounded-xl border px-4 py-2.5" style="border-color: var(--border-subtle)" />
                </div>

                <div>
                    <label class="mb-1 block text-sm font-semibold">صورة المستمسك (اختياري)</label>
                    <input type="file" accept="image/*" class="w-full text-sm" @input="form.id_document_image = $event.target.files[0]" />
                    <p v-if="form.errors.id_document_image" class="mt-1 text-sm text-red-600">{{ form.errors.id_document_image }}</p>
                </div>
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <Link :href="route('app.debtors.index')" class="rounded-pill border-2 px-6 py-2.5 font-bold" style="border-color: var(--border-subtle)">إلغاء</Link>
                </div>
            </form>
        </div>
    </AppLayout>
