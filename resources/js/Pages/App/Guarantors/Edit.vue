...
const props = defineProps({ guarantor: { type: Object, required: true } });

const form = useForm({
    name: props.guarantor.name,
    phone: props.guarantor.phone,
    address: props.guarantor.address ?? '',
    note: props.guarantor.note ?? '',
    id_document_image: null,
    _method: 'put',
});

function submit() {
    form.post(route('app.guarantors.update', props.guarantor.id));
}
</script>
...
<template>
    <AppLayout>
        <div class="mx-auto max-w-lg">
            <h1 class="mb-6 text-lg font-extrabold sm:text-2xl">تعديل بيانات الكفيل</h1>
            ...
            <form class="space-y-4 rounded-card p-6" style="background: var(--surface-card); box-shadow: var(--shadow-card)" @submit.prevent="submit">
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <Link :href="route('app.guarantors.index')" class="rounded-pill border-2 px-6 py-2.5 font-bold" style="border-color: var(--border-subtle)">رجوع</Link>
                </div>
            </form>
        </div>
    </AppLayout>
